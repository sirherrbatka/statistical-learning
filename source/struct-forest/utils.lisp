...
(-> euclid-distance (sl.data:single-float-data-matrix fixnum fixnum) single-float)
(defun euclid-distance (data ai bi)
  ...
  (iterate
    (declare (type fixnum i)
             (type single-float result difference))
    (with result = 0.0)
...
(defun relabel-with-pivots (state first-pivot second-pivot)
  (let* ((target-data (struct-target-data state))
    ...
    (declare (type sl.data:single-float-data-matrix target-data result))
    (iterate
      ...
      (if (< first-distance second-distance)
          (setf (sl.data:mref result i 0) 1.0)
          (setf (sl.data:mref result i 0) 0.0))
