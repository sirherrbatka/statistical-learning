  ...
  (iterate
    ...
    (iterate
      ...
      (setf (sl.data:mref result i ii)
            (- (the single-float (sl.data:mref expected i ii))
               (the single-float (sl.data:mref function-output i ii)))))
...
(defmethod loss ((function squared-error-function)
  ...
  (let* ((target-data-width (sl.data:attributes-count target-data))
         (left-sum (make-array target-data-width :element-type 'single-float
                                                 :initial-element 0.0))
         (right-sum (make-array target-data-width :element-type 'single-float
                                                  :initial-element 0.0))
         (length (sl.data:data-points-count target-data))
         (left-count 0)
         (right-count 0))
    (declare (type (simple-array single-float (*)) left-sum right-sum)
...
(defmethod response ((function k-logistic-function)
  ...
  (declare (optimize (speed 3) (safety 0)
                     (space 0) (debug 0)
                     (compilation-speed 0))
           (type sl.data:single-float-data-matrix sums expected))
  (iterate
